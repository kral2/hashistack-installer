...
# WAYPOINT INSTALLER - Automated Waypoint Installation
#   Apache 2 License - Copyright (c) 2018  Robert Peteuil  @RobertPeteuil
#
#     Automatically Download, Extract and Install
#        Latest or Specific Version of Waypoint
#
#   from: https://github.com/robertpeteuil/waypoint-installer
...
usage() {
  [[ "$1" ]] && echo -e "Download and Install Waypoint - Latest Version unless '-i' specified\n"
...
getLatest() {
  ...
  case "${nettool}" in
    ...
    wgetjq)
      LATEST_ARR=($(wget -q -O- https://releases.hashicorp.com/index.json 2>/dev/null | jq -r '.waypoint.versions[].version' | sort -t. -k 1,1nr -k 2,2nr -k 3,3nr))
      ;;
    curljq)
      LATEST_ARR=($(curl -s https://releases.hashicorp.com/index.json 2>/dev/null | jq -r '.waypoint.versions[].version' | sort -t. -k 1,1nr -k 2,2nr -k 3,3nr))
      ;;
    # parse version from github API
    wget)
      LATEST_ARR=($(wget -q -O- https://api.github.com/repos/hashicorp/waypoint/releases 2> /dev/null | awk '/tag_name/ {print $2}' | cut -d '"' -f 2 | cut -d 'v' -f 2))
      ;;
    curl)
      LATEST_ARR=($(curl -s https://api.github.com/repos/hashicorp/waypoint/releases 2> /dev/null | awk '/tag_name/ {print $2}' | cut -d '"' -f 2 | cut -d 'v' -f 2))
      ;;
  esac
...
FILENAME="waypoint_${VERSION}_${OS}_${PROC}.zip"
LINK="https://releases.hashicorp.com/waypoint/${VERSION}/${FILENAME}"
SHALINK="https://releases.hashicorp.com/waypoint/${VERSION}/waypoint_${VERSION}_SHA256SUMS"
...
if [[ ! "$cwdInstall" ]]; then
  ...
  ${CMDPREFIX} mv waypoint "$BINDIR" || exit 1
  # CLEANUP AND EXIT
  cd "${TMPDIR}" || exit 1
  rm -rf "${UTILTMPDIR}"
  [[ ! "$STREAMLINED" ]] && echo
  echo "Waypoint Version ${VERSION} installed to ${BINDIR}"
else
  rm -f "$FILENAME" SHAFILE
  echo "Waypoint Version ${VERSION} downloaded"
fi
