...
getLatest() {
  ...
  case "${nettool}" in
    ...
    wget)
      LATEST_ARR=($(wget -q -O- https://api.github.com/repos/hashicorp/packer/releases 2> /dev/null | awk '/tag_name/ {print $2}' | cut -d '"' -f 2 | cut -d 'v' -f 2))
      ;;
    curl)
      LATEST_ARR=($(curl -s https://api.github.com/repos/hashicorp/packer/releases 2> /dev/null | awk '/tag_name/ {print $2}' | cut -d '"' -f 2 | cut -d 'v' -f 2))
      ;;
  esac
