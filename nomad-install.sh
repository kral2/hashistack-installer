...
# NOMAD INSTALLER - Automated Nomad Installation
#   Apache 2 License - Copyright (c) 2018  Robert Peteuil  @RobertPeteuil
#
#     Automatically Download, Extract and Install
#        Latest or Specific Version of Nomad
#
#   from: https://github.com/robertpeteuil/nomad-installer
...
usage() {
  [[ "$1" ]] && echo -e "Download and Install Nomad - Latest Version unless '-i' specified\n"
...
getLatest() {
  ...
  case "${nettool}" in
    ...
    wgetjq)
      LATEST_ARR=($(wget -q -O- https://releases.hashicorp.com/index.json 2>/dev/null | jq -r '.nomad.versions[].version' | sort -t. -k 1,1nr -k 2,2nr -k 3,3nr))
      ;;
    curljq)
      LATEST_ARR=($(curl -s https://releases.hashicorp.com/index.json 2>/dev/null | jq -r '.nomad.versions[].version' | sort -t. -k 1,1nr -k 2,2nr -k 3,3nr))
      ;;
    # parse version from github API
    wget)
      LATEST_ARR=($(wget -q -O- https://api.github.com/repos/hashicorp/nomad/releases 2> /dev/null | awk '/tag_name/ {print $2}' | cut -d '"' -f 2 | cut -d 'v' -f 2))
      ;;
    curl)
      LATEST_ARR=($(curl -s https://api.github.com/repos/hashicorp/nomad/releases 2> /dev/null | awk '/tag_name/ {print $2}' | cut -d '"' -f 2 | cut -d 'v' -f 2))
      ;;
  esac
...
FILENAME="nomad_${VERSION}_${OS}_${PROC}.zip"
LINK="https://releases.hashicorp.com/nomad/${VERSION}/${FILENAME}"
SHALINK="https://releases.hashicorp.com/nomad/${VERSION}/nomad_${VERSION}_SHA256SUMS"
...
if [[ ! "$cwdInstall" ]]; then
  ...
  ${CMDPREFIX} mv nomad "$BINDIR" || exit 1
  # CLEANUP AND EXIT
  cd "${TMPDIR}" || exit 1
  rm -rf "${UTILTMPDIR}"
  [[ ! "$STREAMLINED" ]] && echo
  echo "Nomad Version ${VERSION} installed to ${BINDIR}"
else
  rm -f "$FILENAME" SHAFILE
  echo "Nomad Version ${VERSION} downloaded"
fi
